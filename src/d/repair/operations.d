...
class RepairOperations {
    ...
    bool regenerateInitramfs(ref SystemInfo sysInfo) {
        ...
        try {
            ...
            // For pacman systems, build a command list that includes per-preset
            // commands for every installed preset (e.g. linux-cachyos, linux, etc.)
            // so we don't rely on -P succeeding when one preset is broken.
            string[][] commands;
            if (sysInfo.packageManager == PackageManager.PACMAN) {
                auto presets = detectInstalledPresets(sysInfo);
                // Always try -P first
                commands ~= ["mkinitcpio", "-P"];
                // Then add per-preset fallbacks for each detected preset
                foreach (preset; presets) {
                    commands ~= ["mkinitcpio", "-p", preset];
                }
                // Final fallback for the most common CachyOS kernel if none detected
                if (presets.length == 0) {
                    commands ~= ["mkinitcpio", "-p", "linux-cachyos"];
                    commands ~= ["mkinitcpio", "-p", "linux"];
                }
            } else {
                commands = getInitramfsCommands(sysInfo.packageManager);
            }
            ...
            foreach (command; commands) {
                try {
                    ...
                    if (exitCode == 0) {
                        ...
                        anySuccess = true;
                        // For pacman, once any per-preset command succeeds we can stop
                        // (but let all per-preset commands run so all kernels are covered)
                    } else {
    ...
    private void runBootDiagnostics(ref SystemInfo sysInfo) {
        ...
        if (exists(refindConfPath)) {
            ...
            // refind_linux.conf is created during the bootloader fix step, so it may
            // not exist yet when diagnostics run during the initramfs step.
            ui.printInfo("refind_linux.conf not yet present at: " ~ refindConfPath ~ " (will be created during bootloader fix)");
            Logger.info("refind_linux.conf not found during diagnostics - expected if bootloader not yet fixed");
        }
...
`;
    ...
    /**
     * Detect installed mkinitcpio preset names from /etc/mkinitcpio.d/
     * Returns preset names (without .preset extension), e.g. ["linux-cachyos", "linux"]
     */
    private string[] detectInstalledPresets(ref SystemInfo sysInfo) {
        import std.file : dirEntries, SpanMode;
        import std.path : baseName, stripExtension;

        string presetDir = buildPath(sysInfo.mountPoint, "etc", "mkinitcpio.d");
        string[] presets;

        if (!exists(presetDir)) {
            Logger.warning("mkinitcpio.d not found at: " ~ presetDir);
            return presets;
        }

        try {
            foreach (entry; dirEntries(presetDir, "*.preset", SpanMode.shallow)) {
                string name = stripExtension(baseName(entry.name));
                // Skip presets with capital letters or invalid names - mkinitcpio -p is case-sensitive
                // but we include them all so -P has the right companion fallback list
                presets ~= name;
                Logger.info("Found mkinitcpio preset: " ~ name);
            }
        } catch (Exception e) {
            Logger.warning("Could not scan preset directory: " ~ e.msg);
        }
